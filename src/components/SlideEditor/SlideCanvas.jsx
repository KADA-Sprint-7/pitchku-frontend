...
export default function SlideCanvas({
  ...
  // Dynamic ratio scale calculation to fit parent container perfectly
  useEffect(() => {
    ...
    const updateScale = () => {
      ...
      // Padding margins inside canvas wrapper
      const paddingX = window.innerWidth < 768 ? 16 : 32;
      const paddingY = window.innerWidth < 768 ? 16 : 32;

      const availW = Math.max(cw - paddingX, 240);
      const availH = Math.max(ch - paddingY, 140);
      ...
      setFitScale(Math.min(Math.max(calculatedScale, 0.2), 1.2));
    };
  ...
  return (
    <div
      ref={containerRef}
      className="flex-1 overflow-hidden flex flex-col items-center justify-center bg-slate-100 p-2 md:p-6 relative select-none"
    >
      ...
      <div className="w-full h-full flex items-center justify-center overflow-auto p-1 md:p-2">
        ...
        >
          ...
          >
            ...
            <div className="hidden absolute top-3.5 left-6 right-6 z-20 flex items-center justify-between pointer-events-none">
              {/* Bab Badge */}
              <span
            ...
            {/* Bottom Footer Bar */}
            <div className="hidden absolute bottom-0 left-0 right-0 z-20 h-7 flex items-center justify-between px-6 border-t border-white/5 bg-black/40 backdrop-blur-xs select-none pointer-events-none">
